...
- Java 是什么类型的，值类型还是引用类型？Java 传引用的方式？
- 什么是值传递和引用传递？传值和传引用的区别？
- Java 包装类的作用，什么时候装箱，什么时候拆箱，装箱和拆箱是怎么实现的。
- Java 的基本数据类型有哪些，它们的长度分别是多少，它们占用的字节数分别是多少。
- 浮点型能否精确的表示小数，int 的范围，char 占几个字节，String 是不是基本类型的？String 为什么要是 final 类型的？
